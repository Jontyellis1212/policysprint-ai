...
const restoreWizardStateIfPresent = () => {
  ...
  const firePostHogPolicyGenerateFailedOnce = (
    ...
    const safeError =
      (props.error || "").toString().trim().slice(0, 180) || null;
  ...
  const firePostHogPdfDownloadBlocked = (
    payloadToSend: WizardFormState,
    reason: "signin_required" | "upgrade_required"
  ) => {
    try {
  ...
  const firePostHogPdfDownloadFailed = (
    payloadToSend: WizardFormState,
    error?: string | null
  ) => {
    const safeError =
      (error || "").toString().trim().slice(0, 180) || null;
    try {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      firePostHogPdfDownloadFailed(
        form,
        err?.message ? String(err.message) : "Download error"
      );
  ...
  const progressWidth =
    step === 1 ? "33.333%" : step === 2 ? "66.666%" : "100%";
  const helperText =
    step === 1
                  ...
                  >
                    ...
                    <div className="mt-4 flex flex-col gap-2 md:flex-row md:flex-wrap md:items-center">
                      ...
                      <button
                        type="button"
                        onClick={handleCopy}
                        className="inline-flex items-center justify-center rounded-full border border-slate-700 bg-slate-900/40 px-4 py-2 text-[11px] text-slate-100 hover:bg-slate-900/70 w-full md:w-auto"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-full border border-slate-700 bg-slate-900/40 px-4 py-2 text-[11px] text-slate-100 hover:bg-slate-900/70 w-full md:w-auto disabled:opacity-60"
                      >
                    ...
                    <div className="flex items-center justify-between gap-3 mb-2">
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">
                          ...
                          accounts may see a watermark in preview.
                        </div>
                      </div>
                    </div>
